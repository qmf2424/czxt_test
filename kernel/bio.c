...
binit(void)
{
  ...
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){  // head plugging
...
bget(uint dev, uint blockno)
{
  ...
  int i = blockno % NBUCKET;  // calculate bucket index
  ...
  /*  If not cached. */
  release(&bcache.lock[i]);  // release bucket lock first
  
  // acquire locks in order
  acquire(&bcache.biglock); 
  acquire(&bcache.lock[i]); 

  /* Find from current bucket again. */
  for (b = bcache.head[i].next; b != &bcache.head[i]; b = b->next) {
    if(b->dev == dev && b->blockno == blockno) {
      b->refcnt++;
      ...
      acquiresleep(&b->lock);  // acquire cache Sleep Lock
      return b; 
    }
  }

  /* Find a LRU free block from current bucket. */
  struct buf *b2 = 0; 
  int min_ticks = 0;
  for (b = bcache.head[i].next; b != &bcache.head[i]; b = b->next) {
  ...
  /* Find block from other buckets. */
  for (int j = (i + 1) % NBUCKET; j != i; j = (j + 1) % NBUCKET) {
    acquire(&bcache.lock[j]);  // acquire traversed bucket lock
    ...
    if(b2) {
      ...
      release(&bcache.lock[j]);  // release traversed bucket lock
  ...
  release(&bcache.lock[i]);
  release(&bcache.biglock);
  panic("bget: no buffers");  // no buf available, panic
}
