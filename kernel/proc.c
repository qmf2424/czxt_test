...
freeproc(struct proc *p)
{
  ...
  // delete kernel stack
  if(p->kstack) {
    // get page entry of the kernel stack
    pte_t* pte = walk(p->kpagetable, p->kstack, 0);
    if(pte == 0)
      panic("freeproc: walk");
    kfree((void*)PTE2PA(*pte));  // release physical page
  }
  p->kstack = 0;  // Set the pointer to 0, indicate that the kernel stack has been released
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if (pte & PTE_V) {
      ...
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) {
        ...
        // recursively release page entry
        proc_freewalk((pagetable_t)child);
      }
    }
  }
  kfree((void*)pagetable);  // release current page
}
