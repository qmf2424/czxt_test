...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    /*
      (1) Only if the parent process memory page is writable, all will be set to COW and read-only;
      (2) Otherwise, it's all read-only, but it's not marked as COW, because it's read-only and won't be written
    */
    if (*pte & PTE_W) {
      // set PTE_W to 0
      *pte &= ~PTE_W;
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
	...
	// copy
        ...
        // decrease the reference count of old memory and set pte to 0
        uvmunmap(pagetable, va0, 1, 1);
        // change the physical memory address and set PTE_W to 1
        *pte = (PA2PTE(mem) | flags | PTE_W);
        // set PTE_RSW to 0
        *pte &= ~PTE_RSW;
        // update pa0 to new memory address
        pa0 = (uint64)mem;
      }
    }
